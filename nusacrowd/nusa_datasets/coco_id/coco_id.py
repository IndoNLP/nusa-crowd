...
image_text = (kb, pairs, qa, text, t2t, entailment)
"""
import json
import os
...
_URLS = {
    _DATASETNAME: "https://drive.google.com/uc?export=download&id=1y4COEzzO2nmn1yetOCqxTjNsiSVvRMpv",
}
...
class CocoId(datasets.GeneratorBasedBuilder):
    """TODO: Short description of my dataset. COCO ID is an Image Captioning Dataset containing image URL and its caption along with its tags"""
    ...
    #   - name: should be unique for each dataset config eg. smsa_(source|nusantara)_image_text
    #   - version: option = (SOURCE_VERSION|NUSANTARA_VERSION)
    #   - description: one line description for the dataset
    #   - schema: options = (source|nusantara_image_text)
    #   - subset_id: subset id is the canonical name for the dataset (eg. smsa)
    #  where image_text = (kb, pairs, qa, text, t2t)

    BUILDER_CONFIGS = [
        NusantaraConfig(
            name="coco_id_source",
            version=SOURCE_VERSION,
            description="coco_id source schema",
            schema="source",
            subset_id="coco_id",
        ),
        NusantaraConfig(
            name="coco_id_nusantara_image_text",
            version=NUSANTARA_VERSION,
            description="coco_id Nusantara schema",
            schema="nusantara_image_text",
            subset_id="coco_id",
        ),
    ]

    DEFAULT_CONFIG_NAME = "coco_id_source"
    ...
    def _info(self) -> datasets.DatasetInfo:
        ...
        # In rare cases you may get a dataset that supports multiple tasks requiring multiple schemas. In that case you can define multiple nusantara configs with a nusantara_image_text format.

        # For example nusantara_kb, nusantara_t2t
        elif self.config.schema == "nusantara_image_text":
    ...
    def _split_generators(self, dl_manager: datasets.DownloadManager) -> List[datasets.SplitGenerator]:
        ...
        # base_path = Path(dl_manager.download_and_extract(_URLS[_DATASETNAME]))
        ...
        return [
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN, gen_kwargs={"filepath": base_path}
            ),
            # datasets.SplitGenerator(
    ...
    def _generate_examples(self, filepath: Path, split: str) -> Tuple[int, Dict]:
        ...
        train = [item for item in data if item['split'] == 'train']
        test = [item for item in data if item['split'] == 'test']
        val = [item for item in data if item['split'] == 'val']
        restval = [item for item in data if item['split'] == 'restval']
...
# This allows you to run your dataloader with `python coco_id.py` during development
